...
export default function CommentSection({ videoId }) {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      console.log("📥 [FRONTEND RECEIVE]", res.data.data.docs[0]);
  ...
  const handlePostComment = async (e) => {
    ...
    setPosting(true);
    try {
      const res = await api.post(`/Comment/${videoId}`, {
        content: newComment,
      });
  ...
  const toggleCommentLike = async (commentId) => {
    try {
      // 1. Hit the toggle endpoint (Ensure this route exists in your LikeRoutes)
      await api.post(`/likes/toggle/c/${commentId}`);

      // 2. Update local state so it stays "Red"
      setComments((prev) =>
        prev.map((c) => {
          if (c._id === commentId) {
            return {
      ...
      <div className="space-y-6">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div
              key={comment._id}
              className="group flex gap-4 p-4 hover:bg-white/[0.02] rounded-[1.5rem] transition-colors"
            >
              <img
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="flex items-center gap-4 pt-2">
                  <button
                    onClick={() => toggleCommentLike(comment._id)} // 👈 ADD THIS
