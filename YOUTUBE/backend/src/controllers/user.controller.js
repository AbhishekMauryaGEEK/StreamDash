...
const refreshaccessToken = asynchandler(async (req, res) => {
  ...
  console.log(
    "Secret Loaded?",
    process.env.REFRESH_TOKEN_SECRET ? "YES" : "NO"
  );
...
const forgetpassword = asynchandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      "FAILED to  send  email, system error  please try again."
    );
  }
  return res
    .status(200)
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        {},
        "Password reset successfuly.You can now go to login."
      )
    );
});
const updateAccountDetails = asynchandler(async (req, res) => {
    const { github, twitter, portfolio } = req.body;

    // Use findByIdAndUpdate on the ID attached by verifyJWT
    const user = await User.findByIdAndUpdate(
        req.user?._id,
        {
            $set: {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, user, "Social artifacts updated successfully"));
});
export {
  registerUser,
  loginUser,
