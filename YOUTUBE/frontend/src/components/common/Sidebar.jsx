...
import { Home, PlaySquare, Youtube, BarChart3, Settings, Users } from 'lucide-react'; // Added Users icon
import { NavLink } from 'react-router-dom';

const navItems = [
    { name: 'Home', icon: Home, path: '/' },
    { name: 'Videos', icon: PlaySquare, path: '/videos' },
    // Updated from Subscriptions to Following
            ...
            <nav className="flex-1 py-6 px-3 space-y-2">
                {navItems.map((item) => (
                    <NavLink 
                        key={item.name} 
                        to={item.path} 
                        // Added active styling logic so you know which page you're on
                        className={({ isActive }) => `
